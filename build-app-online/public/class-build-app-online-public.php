...
class Build_App_Online_Public {
    ...
    public function enqueue_styles() {
        ...
        wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/build-app-online-public.css', array(), $this->version, 'all' );
    ...
    public function enqueue_scripts() {
        ...
        wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/build-app-online-public.js', array( 'jquery' ), $this->version, false );
    ...
    public function get_blocks() {
        ...
        $dotapp_blocks = get_option('bao_my_blocks');
            ...
            if(is_array($dotapp_blocks) && isset($_REQUEST['id'])) {
            ...
            if(isset($dotapp_blocks[$id])) {
                $block = $dotapp_blocks[$id];
            } else {
                $block = $dotapp_blocks[array_rand($dotapp_blocks, 1)];
            }

            if($block['blockType'] == 'productList' || $block['blockType'] == 'productGrid' || $block['blockType'] == 'productScroll' || $block['blockType'] == 'productSlider') {

                $link_type = isset($block['linkType']) ? $block['linkType'] : 'product_cat';
                $link_id = isset($block['linkId']) ? $block['linkId'] : 0;

                if($block['linkType'] === 'featured') {
                ...
                } else if($block['linkType'] === 'wishList') {
            ...
            if($block['blockType'] == 'postList' || $block['blockType'] == 'postScroll' || $block['blockType'] == 'postSlider' || $block['blockType'] == 'postListTile') {

                $args = array();
                
                $link_id = isset($block['linkId']) ? $block['linkId'] : 0;
                
                if($block['linkType'] == 'category') {
                    $args['category'] = $link_id;
                } else if($block['linkType'] == 'post_tag') {
                    $args['tax_query'] = array(
                        array(
                            'taxonomy' => 'post_tag',
                            'field'    => 'id',
                ...
                $block['posts'] = $this->get_posts($args);

            }

            if($block['blockType'] == 'storeList' || $block['blockType'] == 'storeScroll' || $block['blockType'] == 'storeSlider' || $block['blockType'] == 'storeListTile') {

                if(isset($block['storeType']) && !empty($block['storeType'])) {
                    global $bao_vendor_type;
                    $bao_vendor_type = $block['storeType'];
                }

                $block['stores'] = $this->get_vendors();

            }

            if($locale != 'en' && isset($data['locale'][$block['title']])) {
                $block['title'] = $data['locale'][$block['title']];
            }

            wp_send_json($block['blocks']);
        }
